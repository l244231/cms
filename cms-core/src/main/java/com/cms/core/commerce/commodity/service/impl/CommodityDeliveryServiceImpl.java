/*
 * @(#)CommodityDeliveryServiceImpl.java
 * Author : Zain.Luo
 * Created Date: 2017年1月8日 
 */
package com.cms.core.commerce.commodity.service.impl;

import org.springframework.stereotype.Service;

import com.cms.core.commerce.commodity.domain.CommodityDeliveryDto;
import com.cms.core.commerce.commodity.service.CommodityDeliveryService;
import com.cms.core.common.service.impl.BaseServiceImpl;

/**
 * @title 商品配送标签service实现类  
 * @author Zain.Luo
 * @version 1.0<br>
 * @history<br>
 * 2017年1月8日 Zain.Luo create file<br>
 * Id:CommodityDeliveryServiceImpl.java,v1.0 2017年1月8日 下午2:54:03
 */
@Service("commodityDeliveryService")
public class CommodityDeliveryServiceImpl extends BaseServiceImpl<CommodityDeliveryDto> implements CommodityDeliveryService{
	
}
